...
export default class ProductController {
    ...
    try {
      ...
      if (!id) {
        return res.status(400).json(
          { erro: "Parâmetro 'id' é obrigatório para busca por ID" }
        );
      }
  ...
  async findUniqueProduct(req: Request, res: Response) {
    try {
      const { filial, referencia } = req.query;

      if (!referencia) {
        return res.status(400).json(
          { erro: "Valores aceitos de referência: codigoGis, numeroMaterial, descricao, deposito, posicaoDeposito" }
        );
      }
      const product = await productService.findUnique(filial as string, referencia as string);
      if (!product) {
        return res.status(404).json({ erro: "Produto não encontrado" });
      }
      return res.status(200).json(product);
  ...
  async createProduct(req: Request, res: Response) {
    try {
      ...
      const findProduct = await productService.findByFilialAndCodigoGis(
        filial, codigoGis
      )
  ...
  async updateProduct(req: Request, res: Response) {
    try {
      ...
      const finalPayload = Object.fromEntries(
        Object.entries(updatePayload).filter(([_, value]) => value !== "")
      );
  ...
  async autocompleteProduct(req: Request, res: Response) {
    try {
      const { filial, referencia } = req.query;
      if (!referencia || (referencia as string).length < 2) {
        return res.status(200).json([]);
      }
      ...
      const products = await productService.autocomplete(
        filial as string,
