...
async function getImageFile(id) {
  const image = await productImageService2.findById(id);
  if (!image) {
...
async function updateImageFile(id, req, res) {
  try {
    const oldImage = await productService2.findById(id);
    ...
    const result = oldImageExists ? await productImageService3.update(id, data) : await productImageService3.create({ ...data, productId: id });
...
var ProductImageController = class {
  ...
  async getDataImage(req, res) {
    try {
      const { id } = req.params;
      const image = await productImageService4.findById(id);
      if (!image) {
...
router2.post("/api/create/image/:id", upload.single("buffer"), authMiddleware, requireRole("ADMIN"), (req, res) => {
  productImageController.createImage(req, res);
});
router2.get("/api/find/image/:id", authMiddleware, (req, res) => {
  productImageController.getImage(req, res);
});
router2.get("/api/img/:id", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productImageController.getDataImage(req, res);
});
router2.put("/api/update/image/:id", upload.single("buffer"), authMiddleware, requireRole("ADMIN"), (req, res) => {
  productImageController.updateImage(req, res);
});
router2.delete("/api/remove/image/:id", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productImageController.deleteImage(req, res);
});
var productImageRouter_default = router2;
...
app.use("/imagens", import_express3.default.static(imagesDir));
var __filename = (0, import_url.fileURLToPath)(import_meta.url);
var __dirname = (0, import_path5.dirname)(__filename);
app.use(import_express3.default.static(import_path4.default.join(__dirname, "..", "Tke_Front_end")));
routes_default(app);
app.get("/", (req, res) => {
...
async function removeImageFile(id) {
  ...
  await productImageService5.delete(id);
}
...
var ProductController = class {
  async findById(req, res) {
    try {
      ...
      const product = await productService3.findById(id);
      if (!product) {
  ...
  async findProduct(req, res) {
    try {
      const products = await productService3.findAll();
  ...
  async findUniqueProduct(req, res) {
    try {
      ...
      const product = await productService3.findUnique(filial, referencia);
      if (!product) {
        return res.status(404).json({ erro: "Produto n\xE3o encontrado" });
      }
      return res.status(200).json(product);
  ...
  async createProduct(req, res) {
    try {
      ...
      const findProduct = await productService3.findByFilialAndCodigoGis(filial, codigoGis);
      if (findProduct) {
        return res.status(200).json({
          message: "Produto j\xE1 cadastrado para essa filial"
        });
      }
      const newProduct = await productService3.create({
        filial,
        codigoGis,
        numeroMaterial,
        descricao,
  ...
  async updateProduct(req, res) {
    try {
      ...
      await productService3.update(id, finalPayload);
      return res.status(200).json({
  ...
  async deleteProduct(req, res) {
    try {
      ...
      if (idImg) {
        try {
          const image = await productImageService6.findById(idImg);
          if (image) {
      ...
      await productService3.delete(id);
  ...
  async autocompleteProduct(req, res) {
    try {
      ...
      const products = await productService3.autocomplete(filial, referencia);
  ...
  async exportExcel(req, res) {
    try {
      const produtos = await productService3.findAll();
...
router4.post("/api/product/create", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productController.createProduct(req, res);
});
router4.post("/api/product/upload", authMiddleware, upload2.single("arquivo"), requireRole("ADMIN"), async (req, res) => {
  productController.uploadExcel(req, res);
});
router4.get("/api/product/find/:id", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productController.findById(req, res);
});
router4.get("/api/product/find", authMiddleware, (req, res) => {
  productController.findUniqueProduct(req, res);
});
router4.get("/api/product/autocomplete", authMiddleware, (req, res) => {
  productController.autocompleteProduct(req, res);
});
router4.get("/api/product/allProducts", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productController.findProduct(req, res);
});
router4.put("/api/product/update/:id", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productController.updateProduct(req, res);
});
router4.get("/api/product/export", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productController.exportExcel(req, res);
});
router4.delete("/api/product/remove/:id", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productController.deleteProduct(req, res);
});
var productRouter_default = router4;
