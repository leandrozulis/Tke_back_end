...
var productImageRouter_exports = {};
__export(productImageRouter_exports, {
  default: () => productImageRouter_default
});
module.exports = __toCommonJS(productImageRouter_exports);
...
var ProductController = class {
  ...
  async deleteProduct(req, res) {
    try {
      ...
      if (idImg) {
        try {
          const image = await productImageService2.findById(idImg);
          if (image) {
...
var imagesDir = process.env.VERCEL ? "/tmp/imagens" : import_path2.default.join(process.cwd(), "src", "imagens");
if (!import_fs2.default.existsSync(imagesDir)) {
  import_fs2.default.mkdirSync(imagesDir, { recursive: true });
}
app.use("/imagens", import_express3.default.static(imagesDir));
var __filename = (0, import_url.fileURLToPath)(import_meta.url);
var __dirname = (0, import_path3.dirname)(__filename);
app.use(import_express3.default.static(import_path2.default.join(__dirname, "..", "Tke_Front_end")));
routes_default(app);
app.get("/", (req, res) => {
  res.sendFile(import_path2.default.join(__dirname, "..", "Tke_Front_end", "index.html"));
});
app.get("*", (req, res) => {
  res.sendFile(import_path2.default.join(__dirname, "..", "Tke_Front_end", "index.html"));
});
...
async function addImageFile(id, req, res) {
  try {
    ...
    const filePath = import_path4.default.join(imagesDir, fileName);
    import_fs3.default.writeFileSync(filePath, req.file.buffer);
    await productImageService3.create({
      imagem: fileName,
      productId: id
    });
...
async function getImageFile(id) {
  ...
  const filePath = import_path5.default.join(imagesDir, import_path5.default.basename(relativePath));
  if (!import_fs4.default.existsSync(filePath)) {
    throw new Error("Arquivo f\xEDsico n\xE3o encontrado");
  }
  const file = import_fs4.default.readFileSync(filePath);
...
async function updateImageFile(id, req, res) {
  try {
    ...
    if (oldImageExists) {
      ...
      const oldFilePath = import_path6.default.join(imagesDir, oldFileName);
      if (import_fs5.default.existsSync(oldFilePath)) {
        import_fs5.default.unlinkSync(oldFilePath);
      }
    }
    const file = req.file;
    const newName = `${Date.now()}-${file.originalname}`;
    const newFilePath = import_path6.default.join(imagesDir, newName);
    import_fs5.default.writeFileSync(newFilePath, file.buffer);
    const data = {
      imagem: newName
    };
    const result = oldImageExists ? await productImageService5.update(id, data) : await productImageService5.create({ ...data, productId: id });
...
router4.post("/api/create/image/:id", upload2.single("buffer"), authMiddleware, requireRole("ADMIN"), (req, res) => {
  productImageController.createImage(req, res);
});
router4.get("/api/find/image/:id", authMiddleware, (req, res) => {
  productImageController.getImage(req, res);
});
router4.get("/api/img/:id", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productImageController.getDataImage(req, res);
});
router4.put("/api/update/image/:id", upload2.single("buffer"), authMiddleware, requireRole("ADMIN"), (req, res) => {
  productImageController.updateImage(req, res);
});
router4.delete("/api/remove/image/:id", authMiddleware, requireRole("ADMIN"), (req, res) => {
  productImageController.deleteImage(req, res);
});
var productImageRouter_default = router4;
