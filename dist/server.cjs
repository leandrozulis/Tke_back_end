...
async function addImageFile(id, req, res) {
  try {
    ...
    const filePath = import_path2.default.join(imagesDir, fileName);
    import_fs2.default.writeFileSync(filePath, req.file.buffer);
...
async function getImageFile(id) {
  ...
  const filePath = import_path3.default.join(imagesDir, import_path3.default.basename(relativePath));
  if (!import_fs3.default.existsSync(filePath)) {
    throw new Error("Arquivo f\xEDsico n\xE3o encontrado");
  }
  const file = import_fs3.default.readFileSync(filePath);
...
async function updateImageFile(id, req, res) {
  try {
    ...
    if (oldImageExists) {
      ...
      const oldFilePath = import_path4.default.join(imagesDir, oldFileName);
      if (import_fs4.default.existsSync(oldFilePath)) {
        import_fs4.default.unlinkSync(oldFilePath);
      }
    }
    const file = req.file;
    const newName = `${Date.now()}-${file.originalname}`;
    const newFilePath = import_path4.default.join(imagesDir, newName);
    import_fs4.default.writeFileSync(newFilePath, file.buffer);
    const data = {
      imagem: newName
    };
...
var imagesDir = process.env.VERCEL ? "/tmp/imagens" : import_path5.default.join(process.cwd(), "src", "imagens");
if (!import_fs5.default.existsSync(imagesDir)) {
  import_fs5.default.mkdirSync(imagesDir, { recursive: true });
}
app.use("/imagens", import_express4.default.static(imagesDir));
var __filename = (0, import_url.fileURLToPath)(import_meta.url);
var __dirname = (0, import_path6.dirname)(__filename);
app.use(import_express4.default.static(import_path5.default.join(__dirname, "..", "Tke_Front_end")));
routes_default(app);
app.get("/", (req, res) => {
  res.sendFile(import_path5.default.join(__dirname, "..", "Tke_Front_end", "index.html"));
});
app.get("*", (req, res) => {
  res.sendFile(import_path5.default.join(__dirname, "..", "Tke_Front_end", "index.html"));
});
...
// dist/server.js
var PORTA = Number(process.env.PORTA) || 3e3;
app_default.listen(PORTA, "0.0.0.0", () => {
